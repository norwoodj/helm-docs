...
func TestMustBeEqual(t *testing.T) {
	contentOld, err := readDocumentationInfoByChartPath("../../pkg/helm/test-fixtures", 1)
	if err != nil {
		t.Fatal(err)
	}
	contentNew, err := readDocumentationInfoByChartPath("../../pkg/helm/test-fixtures", 1)
	if err != nil {
		t.Fatal(err)
	}

	eq := reflect.DeepEqual(contentOld, contentNew)
	if eq {
