...
func TestParallelProcessIterableNoError(t *testing.T) {
	items := []string{"a", "b", "c"}
	err := parallelProcessIterable(items, 2, func(elem interface{}) error {
		return nil
	})
	if err != nil {
...
func TestParallelProcessIterableAggregatesErrors(t *testing.T) {
	items := []string{"a", "b", "c"}
	err := parallelProcessIterable(items, 2, func(elem interface{}) error {
		return fmt.Errorf("failed: %v", elem)
	})
	if err == nil {
	...
	for _, item := range items {
		if !strings.Contains(err.Error(), "failed: "+item) {
			t.Errorf("expected error for %q in combined error %q", item, err.Error())
		}
	}
}

func TestHelmDocsReturnsErrorOnInvalidTemplate(t *testing.T) {
	if err := viper.BindFlagValues(testFlagSet{
		"chart-search-root":          filepath.Join("testdata", "invalid-template"),
		"template-files":             "README.md.gotmpl",
		"log-level":                  "warn",
		"ignore-file":                ".helmdocsignore",
		"output-file":                "README.md",
	...
	err := helmDocs(nil, nil)
	if err == nil {
		t.Error("expected error for invalid template, got nil")
	}
}

func TestHelmDocsReturnsErrorOnBadChartRoot(t *testing.T) {
	if err := viper.BindFlagValues(testFlagSet{
		"chart-search-root":          "/nonexistent/path",
		"log-level":                  "warn",
		"ignore-file":                ".helmdocsignore",
		"output-file":                "README.md",
		"values-file":                "values.yaml",
	...
	err := helmDocs(nil, nil)
	if err == nil {
		t.Error("expected error for nonexistent chart-search-root, got nil")
	}
}
