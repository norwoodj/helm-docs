...
func newHelmDocsCommand(run func(cmd *cobra.Command, args []string)) (*cobra.Command, error) {
	...
	command.PersistentFlags().StringP("search-path", "s", ".", "The path to search for chart directories")
	command.PersistentFlags().Bool("omit-blanks", false, "Do not output fields with no description")
	...
	command.PersistentFlags().StringP("container-default-text", "c", "", "For lists / dictionaries output the specified text instead of the whole object")
